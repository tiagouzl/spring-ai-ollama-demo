...
A mock that returns `sim` on demand proves the plumbing, never the judgement. A guardrail that appears healthy — no errors, no trips, code covered by tests — is more dangerous than one that is visibly off: an operator enables it, sees green dashboards, and removes the manual review it was standing in for. The tests and the type system could not have caught this; only the real model could. The fix in commit `bc0ac97` did not make the judge discriminate (no model on this stack does); it made the failure visible — `temperature: 0` plus one log line per verdict. The same lesson recurred for the embedding stage in commit `a72bccb`: an opt-in cosine classifier whose unit and IT tests both passed, until an end-to-end request against the real model showed a raw destructive command (`rm -rf ~/`, cosine 0.63) slipping past narrative examples that scored 0.81 when wrapped in prose. The calibration measured *form*, not content.

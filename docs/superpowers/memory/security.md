...
- **Embedding guardrail** (`app.guardrails.embeddings.enabled=false`, `threshold=0.70`): deterministic third stage — cosine similarity between the answer and labelled violation examples from `src/main/resources/guardrail/policy-examples.json`, embedded once (lazily) with the app's `nomic-embed-text`. One embedding call, no generation. It is **on by default**: the labelled examples cover both forms a violation takes — atomic (`rm -rf ~/`, `DROP TABLE users;`, a bare `sk-proj-…`) and narrative — which is what lets it catch a raw destructive command, not just prose. Calibrated against the real model (violations 0.740–1.000, benign 0.474–0.686, threshold 0.71) and verified end-to-end: a destructive-command request is redacted, near-miss benign answers pass. `EmbeddingPolicyCalibrationIT` re-checks it (`E2E_GUARDRAIL=1 ./mvnw test -Dtest=EmbeddingPolicyCalibrationIT`) — its `rawCommandViolationIsCaught` control is the one that regresses if the examples are trimmed back to narrative-only. Tuning is editing `policy-examples.json`. Fails open like the other stages (`.errors`); trips on `.triggered`. Stage order: blocklist → embeddings → LLM judge.
...
- `ChatModelGuardrailBeanPostProcessor` substitutes the bean's object, so a decorated bean must be injected as the interface it satisfies plus `@Qualifier("<beanName>")` — never by its concrete type. `AlibabaChatController` selects `dashScopeChatModel` this way; a `DashScopeChatModel`-typed injection breaks startup whenever a key is set, and `@MockitoBean` hides the break (mocks bypass post-processing). `DashScopeGuardedTest` uses the real auto-configured bean to keep it covered.

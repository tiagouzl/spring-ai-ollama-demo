...
- **Embedding guardrail** (`app.guardrails.embeddings.enabled=false`, `threshold=0.75`): deterministic third stage — cosine similarity between the answer and labelled violation examples from `src/main/resources/guardrail/policy-examples.json`, embedded once (lazily) with the app's `nomic-embed-text`. One embedding call, no generation. Threshold is calibrated, not guessed: measured positives 0.70–0.98 vs negatives 0.49–0.67, so 0.75 sits above the gap; the separation is narrow, so re-check with `E2E_GUARDRAIL=1 ./mvnw test -Dtest=EmbeddingPolicyCalibrationIT` when the model or examples change. Fails open like the other stages (`.errors`); trips on `.triggered`. Stage order: blocklist → embeddings → LLM judge.

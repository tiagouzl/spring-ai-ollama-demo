...
- **Embedding guardrail** (`app.guardrails.embeddings.enabled=true`, `threshold=0.70`): deterministic third stage — cosine similarity between the answer and labelled violation examples from `src/main/resources/guardrail/policy-examples.json`, embedded once (lazily) with the app's `nomic-embed-text`. One embedding call, no generation. **On by default**: measured over two independent pools (0 false positives, 6/6 violations the blocklist misses); benign 0.474–0.672, violations 0.727–0.982. The threshold is calibrated, not guessed — re-check with `E2E_GUARDRAIL=1 ./mvnw test -Dtest=EmbeddingPolicyCalibrationIT` (its near-miss test locks the default) when the model or examples change. Fails open like the other stages (`.errors`); trips on `.triggered`. Stage order: blocklist → embeddings → LLM judge. The stages are complementary, not redundant: the README's own jailbreak prompt is caught by the blocklist at 0.561 on the embedding scale.
